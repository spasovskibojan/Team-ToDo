...
namespace IT_Proekt_Proba_Teams.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult AddUserToRole()
        {
            ...
            model.Roles = new List<string>() { "Administrator", "TeamLeader", "Employee"};
            return View(model);
        }
        [HttpPost]
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    IdentityResult addRoleResult = UserManager.AddToRole(user.Id, "Employee");

                    if (addRoleResult.Succeeded)
                    {
                        // Улогата е успешно доделена
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        var addRoleResult = await UserManager.AddToRoleAsync(user.Id, "Employee");
                        if (addRoleResult.Succeeded)
                        {
                            await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                            return RedirectToLocal(returnUrl);
